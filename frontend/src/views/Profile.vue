    ...
    <el-container>
      ...
      <el-main class="main">
        ...
        <div v-if="activeMenu === 'lost'">
          ...
          <el-table :data="lostList" border stripe v-loading="loading">
        ...
        <!-- 我发布的招领 -->
        <div v-else>
          <h2 style="margin-bottom: 16px">我发布的招领</h2>

          <!-- <div class="action-bar" style="margin-bottom: 16px">
            <el-button type="primary" @click="goToPublishFound">
              <el-icon><Plus /></el-icon>
              发布新招领
            </el-button>
          </div> -->

          <el-table :data="foundList" border stripe v-loading="foundLoading">
            <el-table-column prop="title" label="标题" />
            <el-table-column prop="category" label="类别" />
            <el-table-column prop="foundLocation" label="拾到地点" />
            <el-table-column prop="foundTime" label="拾到时间" />
...
/* 招领相关状态 - 在 lostList 下方添加 */
const foundList = ref([]);
const foundLoading = ref(false);

/* 导入相关图标 - 在 ArrowDown 导入下方添加 */
import { Plus } from '@element-plus/icons-vue';

/* 导入招领API - 在 user API 导入下方添加 */
import { getMyFoundItems, deleteFoundItem } from '../api/found';

/* 获取我发布的招领 - 在 fetchMyLostItems 下方添加 */
...
/* 招领相关操作 - 在 editLost/deleteLost 下方添加 */
const editFound = (id) => {
